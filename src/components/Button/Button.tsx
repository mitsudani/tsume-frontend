...
const Button = ({ children, className, variant = ButtonVariant.Primary, ...attributes }: ButtonProps): JSX.Element => (
    <button className={`${styles.button} ${stylesMap[variant]} ${className}`} {...attributes}>
        {children}
    </button>
);
